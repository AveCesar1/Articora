...
function migrateListViewTracking() {
    console.log('\n👁️  Verificando esquema de vistas de listas...');

    try {
        const tableExists = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='list_views'").get();
        if (!tableExists) {
            db.exec(`
            ...
            `);
            console.log('✅ Tabla list_views creada');
        } else {
            console.log('📋 Tabla list_views ya existe');
        }
        ...
        `);
        console.log('✅ Índices de vistas de listas verificados');
    } catch (err) {
        console.error('⚠️ Error migrando list_views:', err && err.message);
    }
}
