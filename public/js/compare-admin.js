...
const metadataFields = [
    ...
    { key: 'keywords', label: 'Palabras clave', format: (val) => val && Array.isArray(val) ? val.join(', ') : '-' }, //undefined is not an object (evaluating 'val.join'). What do we do?
    { key: 'url', label: 'URL' },
...
function init() {
    ...
    // If the server provided preselected IDs (via ?ids=...), apply them and auto-show comparison
    try {
        const preIds = (window.preselectedIds && Array.isArray(window.preselectedIds)) ? window.preselectedIds.slice() : [];
        const preSources = (window.preselectedSources && Array.isArray(window.preselectedSources)) ? window.preselectedSources.slice() : [];
        if (preIds.length) {
            // Ensure any full objects are present in availableSources
            preSources.forEach(ps => {
                if (!state.availableSources.some(s => s.id === ps.id)) {
                    state.availableSources.unshift(ps);
                }
            });

            // Add missing ids by fetching if necessary
            const missing = preIds.filter(id => !state.availableSources.some(s => s.id === id));
            if (missing.length) {
                fetch('/api/admin/compare/sources?ids=' + missing.join(','))
                    .then(r => r.json())
                    .then(json => {
                        if (json && json.success && Array.isArray(json.sources)) {
                            json.sources.forEach(s => { if (!state.availableSources.some(x => x.id === s.id)) state.availableSources.unshift(s); });
                            renderAvailableSources(state.availableSources);
                            // add to selection
                            preIds.forEach(id => {
                                if (!state.selectedSources.some(s => s.id === id)) {
                                    const src = getSourceById(id);
                                    if (src) state.selectedSources.push(src);
                                }
                            });
                            updateUI();
                        ...
                        // fallback: still try to select what we have
                        preIds.forEach(id => {
                            if (!state.selectedSources.some(s => s.id === id)) {
                                const src = getSourceById(id);
                                if (src) state.selectedSources.push(src);
    ...
    } catch (e) { console.warn('apply preselected failed', e); }
}
...
function setupEventListeners() {
    ...
    // Comparar metadatos -> redirect to server with ids so page reloads and opens comparison
    if (elements.compareMetadata) elements.compareMetadata.addEventListener('click', handleCompareClick);
...
// When user clicks Compare, navigate to /compare/admin?ids=...
function handleCompareClick(e) {
    ...
    const ids = state.selectedSources.map(s => s.id).join(',');
    // Navigate to same page with ids as query param so server can pre-load and the page will auto-open
    window.location.href = '/compare/admin?ids=' + encodeURIComponent(ids);
}
...
function confirmMerge() {
    ...
    // Real merge: call server API
    showToast('Enviando solicitud de fusión al servidor...', 'info');

    const payload = {
        base_id: state.selectedForMerge.base,
        ids_to_merge: [state.selectedForMerge.base, ...state.selectedForMerge.others],
        reason: ''
    };

    fetch('/api/admin/compare/merge', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload)
    }).then(r => r.json()).then(result => {
        if (!result || !result.success) {
            showToast('Error al realizar la fusión: ' + (result && result.message ? result.message : 'error desconocido'), 'danger');
            return;
        }

        showToast(`Fusión completada: Fuente #${state.selectedForMerge.base} actualizada con datos de ${result.merged || 0} fuente(s)`, 'success');

        // Remove merged sources from client state
        const removedIds = [state.selectedForMerge.base, ...state.selectedForMerge.others];
        state.availableSources = state.availableSources.filter(s => !removedIds.includes(s.id));
        state.selectedSources = state.selectedSources.filter(s => !removedIds.includes(s.id));

        // Reset merge selection
        ...
        showToast('Error de red al intentar fusionar', 'danger');
    });
}
