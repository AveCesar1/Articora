...
// ========================================
// PANEL PRINCIPAL DE ADMINISTRACIÓN (/admin)
// ========================================
...
app.get('/admin', (req, res) => {
    ...
    const stats = {
        totalPending: manualReports.filter(r => r.status === 'pendiente').length + 
                     systemReports.filter(r => r.status === 'pendiente').length,
        pendingManual: manualReports.filter(r => r.status === 'pendiente').length,
        pendingSystem: systemReports.filter(r => r.status === 'pendiente').length,
        highPriority: manualReports.filter(r => r.priority === 'alta' && r.status === 'pendiente').length,
        resolvedToday: 3,
