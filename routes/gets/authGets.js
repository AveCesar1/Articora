    ...
    app.get('/profile', IsRegistered, (req, res) => {
        try {
            ...
            const userRow = db.prepare(
                `SELECT id, username, profile_picture, bio, institution, academic_level, available_for_messages, is_validated, created_at, first_name, last_name
                 FROM users WHERE id = ?`
            ).get(userId);
            const interestsRows = db.prepare('SELECT interest FROM user_interests WHERE user_id = ?').all(userId);
            const interests = (interestsRows && interestsRows.length) ? interestsRows.map(r => r.interest) : [];
